...
GenoFgenreation <- function(data){
  Notmethylated_sample <- readRDS("/home/kunqi/m6A reader/eIF3a/Full/negative1.rds")
  mature_sample <- readRDS("/home/kunqi/m6A reader/eIF3a/Full/postive.rds")
  analysis_data <- data
  
  mcols(mature_sample) <- NULL
  analysis_data <- c(analysis_data,mature_sample,Notmethylated_sample)
  ...
  top_selected_final <- readRDS("/home/yuxuan.wu/m6A reader/top_selected_final/feature_eIF3a_Full.rds")
...
SitePredictionCollection <- function(data){
  ...
  reModel2 <- readRDS("/home/yuxuan.wu/m6A reader/model/eIF3a_Full.rds")
