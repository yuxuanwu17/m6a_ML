...
for (method in tst_all){
  ...
  setwd("~/testing")
  aucName <- paste("aucMatrix",method,"-tstallFull78030.rds",sep = "")  
...
saveRDS(FullYTHDF3_StoreMatrix,"storeMatrix_FullYTHDF3_tstall78030.rds")
...
index_train <- grep(1,FullYTHDF3_Pos$GSE78030)
...
for (method in genome_method){
  for(i in 1:10){
    ...
    if (method=="genome_chemiPro"){
      ...
      try(pred_result_auc2 <- performance(pred_result2,"auc"))
      try(aucMatrix[i,] <-attr(pred_result_auc2,"y.values")[[1]][1])
      saveRDS(reModel2,"/home/yuxuan.wu/m6A reader/model/YTHDF3_full78030.rds")
    }
