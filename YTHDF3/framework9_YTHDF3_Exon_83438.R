...
index_train <- grep(0,ExonYTHDF3_Pos$GSE83438)
ExonYTHDF3_Pos_indep <- ExonYTHDF3_Pos[index_train]

# testing_sample
index_testing <- grep(1,ExonYTHDF3_Pos$GSE83438)
...
encoding<-c("genome_chemiPro","CONPOSITION","Chemiprop_SeqFeature","EIIP","AutoCo_PseKNC","AutoCo","PSNP","genome","onehot")

format=""
num=0
for (method in encoding){
  # for (method in c("AutoCo","PSNP","genome","onehot")){
  num <- num+1
  try(for (i in 1:10){
...
# saveRDS(ExonYTHDF3_StoreMatrix,"storeMatrix_ExonYTHDF3_83438.rds")
