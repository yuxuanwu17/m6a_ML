...
testT_testing_genome <- FulleIF3a_Pos[index]

# positive sample (all)
testT<-as.character(DNAStringSet(Views(Hsapiens,FulleIF3a_Pos[-index]+20)))
testT_genome <- FulleIF3a_Pos[-index]
...
for (method in tst_all){
  ...
  setwd("~/testing")
  aucName <- paste("aucMatrix",method,"-tstallFull65004.rds",sep = "")  
...
saveRDS(FulleIF3a_StoreMatrix,"storeMatrix_FulleIF3a_tstall65004.rds")
