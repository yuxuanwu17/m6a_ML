...
# get data
ExonYTHDF3_p <- readRDS("/home/kunqi/m6A reader/YTHDF3/Exon/postive.rds")
p_index <- grep(1,ExonYTHDF3_p$GSE86214)
training_ExonYTHDF3_p <- ExonYTHDF3_p[-p_index]

read_neg <- readRDS(paste("/home/kunqi/m6A reader/YTHDF3/Exon/negative",1,".rds",sep = ""))
training_ExonYTHDF3_n <- read_neg[-p_index]

matureSE <- SummarizedExperiment()
mcols(training_ExonYTHDF3_p) <- NULL
rowRanges(matureSE) <- c(training_ExonYTHDF3_p,training_ExonYTHDF3_n-2) # -2!
...
label_train <- c(rep("P",(length(training_ExonYTHDF3_p))),rep("N",(length(training_ExonYTHDF3_n))))
...
saveRDS(top_selected_final, "feature_YTHDF3_Exon.rds") # you have to save it !
...
ggsave("auroc_Exon_YTHDF3.pdf",VarMresult)
...
# testing_sample
index<- grep(1,ExonYTHDF3_Pos$GSE86214)
...
saveRDS(ExonYTHDF3_StoreMatrix,"ExonYTHDF3_StoreMatrix_genome_86214.rds")
