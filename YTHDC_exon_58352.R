...
# positive_Exon
ExonYTHDC1_Pos <- readRDS("/home/kunqi/m6A reader/YTHDC1/Exon/postive.rds")

# testing_sample
index<- grep(1,ExonYTHDC1_Pos$GSE58352)
testT_testing <- as.character(DNAStringSet(Views(Hsapiens,ExonYTHDC1_Pos[index]+20)))
testT_testing_genome <- ExonYTHDC1_Pos[index]

# positive sample (all)
testT<-as.character(DNAStringSet(Views(Hsapiens,ExonYTHDC1_Pos[-index]+20)))
testT_genome <- ExonYTHDC1_Pos[-index]
...
ExonYTHDC1_StoreMatrix <- matrix(NA, nrow=9, ncol=2) 
rownames(ExonYTHDC1_StoreMatrix) <- c("genome_chemiPro","CONPOSITION","Chemiprop_SeqFeature","EIIP","AutoCo_PseKNC","AutoCo","PSNP","genome","onehot")
colnames(ExonYTHDC1_StoreMatrix) <- c("crossValid","indepTest")
...
for (method in tst4){
  for (i in 1:10){
    ...
    storeNegMatrix[i,] <- paste("/home/kunqi/m6A reader/YTHDC1/Exon/negative",i,".rds",sep = "")  
  ...
  setwd("~/testing")
  aucName <- paste("aucMatrix",method,"-tstallexon58352.rds",sep = "")  
  ...
  ExonYTHDC1_StoreMatrix[method,] <- results
}

setwd("/home/yuxuan.wu/m6A reader/storeMatrix")
saveRDS(ExonYTHDC1_StoreMatrix,"storeMatrix_ExonYTHDC1_tstall58352.rds")
