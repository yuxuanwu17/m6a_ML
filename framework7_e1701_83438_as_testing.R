...
index_train <- grep(0,ExonYTHDF3_Pos$GSE83438)   ##### this is the training set as positive
ExonYTHDF3_Pos_indep <- ExonYTHDF3_Pos[index_train]

# testing_sample 
index_testing <- grep(1,ExonYTHDF3_Pos$GSE83438) #### this is the testing set as positive
...
for (method in c("CONPOSITION","Chemiprop_SeqFeature","EIIP","AutoCo_PseKNC","AutoCo","PSNP","genome","genome_chemiPro","onehot")){
  ...
  for (i in 1:10){
    ...
    testN_read_character <- as.character(DNAStringSet(Views(Hsapiens,testN_read+18)))
    testN_testing <- sample(testN_read_character, size = length(testT_testing))
