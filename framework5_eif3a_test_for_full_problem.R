...
FulleIF3a_StoreMatrix <- matrix(NA, nrow=3, ncol=2) 
rownames(FulleIF3a_StoreMatrix) <- c("PSNP","genome","onehot")
colnames(FulleIF3a_StoreMatrix) <- c("crossValid","indepTest")

num=0
for (method in c("PSNP","genome","onehot")){
  num <- num+1
  for (i in 1:10){
    ...
    if (method =="Chemiprop+SeqFeature"){
...
saveRDS(FulleIF3a_StoreMatrix,"storeMatrix_FulleIF3a_0913_test.rds")

# saveRDS(FulleIF3a_StoreMatrix,"storeMatrix.rds")
# FulleIF3a_StoreMatrix<- readRDS("storeMatrix.rds")
# FulleIF3a_StoreMatrix <- readRDS("storeMatrix_FulleIF3a_0912.rds")
