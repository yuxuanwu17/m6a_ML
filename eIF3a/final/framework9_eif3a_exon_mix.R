...
for(method in tst_all){
  for (i in 1:10){
    ...
    Cmotifvsnon_pred <- train(class ~ ., data = training, 
                                ...
                                metric = "ROC")
      testing$pred_class <- predict(Cmotifvsnon_pred, testing, type="prob")
      BIOmotifvsnon_testppred <- prediction(testing$pred_class$Pos,testing$class)
      ...
      saveRDS(Cmotifvsnon_pred,"/home/yuxuan.wu/m6A reader/model/eIF3a_exon_mix.rds")
...
saveRDS(ExoneIF3a_StoreMatrix,"ExoneIF3a_StoreMatrix_mix.rds")
