...
# get data
ExonYTHDF1_p <- readRDS("/home/kunqi/m6A reader/YTHDF1/Exon/postive.rds")
read_neg <- readRDS(paste("/home/kunqi/m6A reader/YTHDF1/Exon/negative",1,".rds",sep = ""))

matureSE <- SummarizedExperiment()
mcols(ExonYTHDF1_p) <- NULL
rowRanges(matureSE) <- c(ExonYTHDF1_p,read_neg-2) # -2!
...
label_train <- c(rep("P",(length(ExonYTHDF1_p))),rep("N",(length(read_neg))))
...
Varplot
ggsave("/home/yuxuan.wu/m6A reader/auroc/AUROC_nomal_f_score_YTHDF1_Exon.pdf",Varplot,width = 8.63, height = 4.7)
...
saveRDS(top_selected_final, "feature_YTHDF1_Exon.rds") # you have to save it !
saveRDS(ROC_max,"ROC_max_YTHDF1_Exon.rds")
...
ggsave("auroc_Exon_YTHDF1_mix.pdf",VarMresult,width = 8.63, height = 4.7)
