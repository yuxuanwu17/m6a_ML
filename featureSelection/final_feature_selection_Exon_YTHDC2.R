...
# get data
ExonYTHDC2_p <- readRDS("/home/kunqi/m6A reader/YTHDC2/Exon/postive.rds")
read_neg <- readRDS(paste("/home/kunqi/m6A reader/YTHDC2/Exon/negative",1,".rds",sep = ""))

matureSE <- SummarizedExperiment()
mcols(ExonYTHDC2_p) <- NULL
rowRanges(matureSE) <- c(ExonYTHDC2_p,read_neg-2) # -2!
...
label_train <- c(rep("P",(length(ExonYTHDC2_p))),rep("N",(length(read_neg))))
...
Varplot
ggsave("/home/yuxuan.wu/m6A reader/auroc/AUROC_nomal_f_score_YTHDC2_Exon.pdf",Varplot,width = 8.63, height = 4.7)
...
saveRDS(top_selected_final, "feature_YTHDC2_Exon.rds") # you have to save it !
saveRDS(ROC_max,"ROC_max_YTHDC2_Exon.rds")
...
ggsave("auroc_Exon_YTHDC2_mix.pdf",VarMresult,width = 8.63, height = 4.7)
