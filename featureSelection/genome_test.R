...
# get data
ExoneIF3a_p <- readRDS("/home/kunqi/m6A reader/eIF3a/Exon/postive.rds")
p_index <- grep(1,mcols(ExoneIF3a_p)[,1])
training_ExoneIF3a_p <- ExoneIF3a_p[-p_index]

read_neg <- readRDS(paste("/home/kunqi/m6A reader/eIF3a/Exon/negative",1,".rds",sep = ""))
training_ExoneIF3a_n <- read_neg[-p_index]

matureSE <- SummarizedExperiment()
mcols(training_ExoneIF3a_p) <- NULL
rowRanges(matureSE) <- c(training_ExoneIF3a_p,training_ExoneIF3a_n-2) # -2!
...
label_train <- c(rep("P",(length(training_ExoneIF3a_p))),rep("N",(length(training_ExoneIF3a_n))))
...
saveRDS(top_selected_final, "feature_eIF3a_exon.rds") # you have to save it !
...
ggsave("auroc_Exon_eIF3a.pdf",VarMresult)
...
saveRDS(ExoneIF3a_StoreMatrix,"ExoneIF3a_StoreMatrix_genome.rds")
