...
# get data
FullYTHDC1_p <- readRDS("/home/kunqi/m6A reader/YTHDC1/Full/postive.rds")
read_neg <- readRDS(paste("/home/kunqi/m6A reader/YTHDC1/Full/negative",1,".rds",sep = ""))

matureSE <- SummarizedExperiment()
mcols(FullYTHDC1_p) <- NULL
rowRanges(matureSE) <- c(FullYTHDC1_p,read_neg-2) # -2!
...
label_train <- c(rep("P",(length(FullYTHDC1_p))),rep("N",(length(read_neg))))
...
Varplot
ggsave("/home/yuxuan.wu/m6A reader/auroc/AUROC_nomal_f_score_YTHDC1_Full.pdf",Varplot,width = 8.63, height = 4.7)
...
saveRDS(top_selected_final, "feature_YTHDC1_Full.rds") # you have to save it !
saveRDS(ROC_max,"ROC_max_YTHDC1_Full.rds")
...
ggsave("auroc_Full_YTHDC1_mix.pdf",VarMresult,width = 8.63, height = 4.7)
