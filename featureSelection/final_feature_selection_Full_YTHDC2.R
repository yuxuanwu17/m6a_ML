...
# get data
FullYTHDC2_p <- readRDS("/home/kunqi/m6A reader/YTHDC2/Full/postive.rds")
read_neg <- readRDS(paste("/home/kunqi/m6A reader/YTHDC2/Full/negative",1,".rds",sep = ""))

matureSE <- SummarizedExperiment()
mcols(FullYTHDC2_p) <- NULL
rowRanges(matureSE) <- c(FullYTHDC2_p,read_neg-2) # -2!
...
label_train <- c(rep("P",(length(FullYTHDC2_p))),rep("N",(length(read_neg))))
...
Varplot
ggsave("/home/yuxuan.wu/m6A reader/auroc/AUROC_nomal_f_score_YTHDC2_Full.pdf",Varplot,width = 8.63, height = 4.7)
...
saveRDS(top_selected_final, "feature_YTHDC2_Full.rds") # you have to save it !
saveRDS(ROC_max,"ROC_max_YTHDC2_Full.rds")
...
ggsave("auroc_Full_YTHDC2_mix.pdf",VarMresult,width = 8.63, height = 4.7)
